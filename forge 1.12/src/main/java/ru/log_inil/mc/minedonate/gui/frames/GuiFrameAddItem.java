...
public class GuiFrameAddItem extends GuiFrame {
    ...
    public void draw(ShopGUI g, int page, int mouseX, int mouseY, float partialTicks, DrawType dt) {
        ...
        if (drawItemStack && MineDonate.getAccount().ms.currentItemStack != null) {
            ...
            g.getItemRender().renderItemAndEffectIntoGUI(MineDonate.getAccount().ms.currentItemStack, posX + 8, posY + 16);
            g.getItemRender().renderItemOverlayIntoGUI(g.getFontRenderer(), MineDonate.getAccount().ms.currentItemStack, posX + 8, posY + 16, Integer.toString(MineDonate.getAccount().ms.currentItemStack.getCount()));
    ...
    public void postShow(ShopGUI g) {
        ...
        cancelChangesButton.y = saveChangesButton.y = posY + height;
        cancelChangesButton.x = posX + width - cancelChangesButton.width;
        saveChangesButton.x = cancelChangesButton.x - saveChangesButton.width;

        if (uppendChangesButton != null) {

            uppendChangesButton.x = saveChangesButton.x - uppendChangesButton.width;
    ...
    public boolean actionPerformed(ShopGUI g, GuiButton b) {
        ...
        if (b.id == saveChangesButton.id) {
            ...
            if (MineDonate.getAccount().canUnlimitedItems()) {
                ...
                limit = MineDonate.getAccount().ms.currentItemStack.getCount();
