...
public abstract class ShopCategory extends GuiEntry {
    ...
    public void draw(ShopGUI g, int page, int mouseX, int mouseY, float partialTicks, DrawType dt) {
        ...
        if (dt == DrawType.BG) {
            ...
            byte var20 = 12;
            /*
             M4G1C NUM8ER$ - V0D00 NUM8ER$ !!
            */
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
            //bufferbuilder.putColor4(0);
            GlStateManager.color(0,0,0,0);
            bufferbuilder.pos((double) 30, (double) ((int) (g.getScaledResolution().getScaledHeight() * 0.1) + 19 + 20 + var20), 0.0D).tex(0.0D, 1.0D).endVertex();
            bufferbuilder.pos((double) (int) g.getScaledResolution().getScaledWidth() - 30, (double) ((g.getScaledResolution().getScaledHeight() * 0.1) + 19 + 20 + var20), 0.0D).tex(1.0D, 1.0D).endVertex();
//            bufferbuilder.putColor4(105);

            bufferbuilder.pos((double) g.getScaledResolution().getScaledWidth() - 30, (double) (int) (g.getScaledResolution().getScaledHeight() * 0.1) + 19 + 20, 0.0D).tex(1.0D, 0.0D).endVertex();
            bufferbuilder.pos((double) 30, (double) (int) (g.getScaledResolution().getScaledHeight() * 0.1) + 19 + 20, 0.0D).tex(0.0D, 0.0D).endVertex();
...
//            bufferbuilder.putColor4(105);
            bufferbuilder = var18.getBuffer();

            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
            bufferbuilder.pos((double) 30, (double) ((g.getScaledResolution().getScaledHeight()) - (g.getScaledResolution().getScaledHeight() * 0.1)) - 5, 0.0D).tex(0.0D, 1.0D).endVertex();
            bufferbuilder.pos((double) g.getScaledResolution().getScaledWidth() - 30, (double) ((g.getScaledResolution().getScaledHeight()) - (g.getScaledResolution().getScaledHeight() * 0.1)) - 5, 0.0D).tex(1.0D, 1.0D).endVertex();

            bufferbuilder.pos((double) g.getScaledResolution().getScaledWidth() - 30, (double) (((g.getScaledResolution().getScaledHeight()) - (g.getScaledResolution().getScaledHeight() * 0.1)) - 5 - var20), 0.0D).tex(1.0D, 0.0D).endVertex();
            bufferbuilder.pos((double) 30, (double) (((g.getScaledResolution().getScaledHeight()) - (g.getScaledResolution().getScaledHeight() * 0.1)) - 5 - var20), 0.0D).tex(0.0D, 0.0D).endVertex();
