    	...
    	if ( e . entityPlayer instanceof EntityPlayerMP ) {
    		...
    		if ( acc != null ) {
    			...
    			if ( acc . ms . mobSelect ) {
    				...
    				/*
    				if ( ! acc . canEditShop ( "SERVER" ) ) {
    				
                		ModNetwork . sendToManageResponsePacket ( ( EntityPlayerMP ) e . entityPlayer, ResponseType . ENTITY, ResponseCode . ADD, ResponseStatus . ERROR_ACCESS_DENIED ) ;
